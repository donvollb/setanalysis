...
bsp.boxplot <- function(x = "default", # Daten, bei "default" wird ein Beispieldatensatz genutzt
                        ...
                        family = set.analysis.defaults$font.family)  # Schriftart
{
  ...
  subchunkify(c(
    ...
    par(family = family),
    boxplot(x, horizontal = TRUE, ylim=c(1,6), col = c(color),
            border="black", xaxt="n", axes = TRUE, pars=list(outcol = color, outpch=20)),
    abline(v=c(1,2,3,4,5,6), col = "gray80"),
    boxplot(x, horizontal = TRUE, ylim=c(1,6), col = c(color) ,
            border="black", xaxt="n", axes = TRUE, pars=list(outcol = color, outpch=20), add = TRUE),
    mtext(c("trifft gar \nnicht zu", "", "", "", "", "trifft voll \nzu"),
          side=1, line=1, at=c(1, 2, 3, 4, 5, 6), las=1, col="gray30", cex=1, font = 2),
    mtext(c("BOXPLOT-BEISPIEL"), side=2, line=2, at=c(1), las=1, col=c(set.analysis.defaults$color.font), cex=1, font = 2),
    par(xpd=TRUE),
    text(x=4.5, y=2,label="Median", col = "grey30"),
    segments(x0 = 4.5, y0 = 1.85, x1 = 5, y1 = 1.2, col = "grey30", lwd = 1),
    text(x=4, y= -0.4,label="6-stufige Skala", col = c(set.analysis.defaults$color.font)),
    segments(x0 = 4.8, y0 = -0.35, x1 = 5.5, y1 = -0.05, col = c(set.analysis.defaults$color.font), lwd = 1),
    segments(x0 = 1.4, y0 = -0.01, x1 = 3.35, y1 = -0.4, col = c(set.analysis.defaults$color.font), lwd = 1),
