...
bsp.boxplot <- function(x = "default", # Daten, bei "default" wird ein Beispieldatensatz genutzt
                        color = settings$color.bars, # Farbe des Boxplots
                        family = settings$font.family)  # Schriftart
{
  ...
  subchunkify(c(
    ...
    mtext(c("BOXPLOT-BEISPIEL"), side=2, line=2, at=c(1), las=1, col=c(settings$color.font), cex=1, font = 2),
    par(xpd=TRUE),
    text(x=4.5, y=2,label="Median", col = "grey30"),
    segments(x0 = 4.5, y0 = 1.85, x1 = 5, y1 = 1.2, col = "grey30", lwd = 1),
    text(x=4, y= -0.4,label="6-stufige Skala", col = c(settings$color.font)),
    segments(x0 = 4.8, y0 = -0.35, x1 = 5.5, y1 = -0.05, col = c(settings$color.font), lwd = 1),
    segments(x0 = 1.4, y0 = -0.01, x1 = 3.35, y1 = -0.4, col = c(settings$color.font), lwd = 1),
